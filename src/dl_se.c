...
void dl_se(dl_se_input_t *input, dl_se_output_t *output)
{
    ...
    double b = 2 * sqrt((m + 1) / (3 * m)) * cos(M_PI / 3 + 1.0 / 3.0 * acos(3 * c / 2 * sqrt(3 * m / pow((m + 1), 3)))); // Eq (24c)
    ...
    else
    {
        ...
        double aem = 500 * pow((input->d / (sqrt(input->hte) + sqrt(input->hre))), 2); // Eq (26)

        // Use the method in Sec. 4.3.3 for adft = aem to obtain Ldft
        dl_se_ft_input.ap = aem;
        dl_se_ft(&dl_se_ft_input, &dl_se_ft_output);

        output->Ldsph[0] = (dl_se_ft_output.Ldft[0] < 0) ? 0 : ((1 - hse / hreq) * dl_se_ft_output.Ldft[0]);
        output->Ldsph[1] = (dl_se_ft_output.Ldft[1] < 0) ? 0 : ((1 - hse / hreq) * dl_se_ft_output.Ldft[1]);
    }
}
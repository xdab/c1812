...
void c1812_calculate(c1812_parameters_t *parameters, c1812_results_t *results)
{
	...
	// Compute b0
	// ctx.b0 = beta0(parameters->lon, ctx.dtm, ctx.dlm);
	...
	pl_los(dtot, hts, hrs, f, p, b0, dlt, dlr, &Lbfs, &Lb0p, &Lb0b);

	// double Ldp, Ldb, Ld50, Lbulla50, Lbulls50, Ldsph50;
	// dl_p(d, g, htc, hrc, hstd, hsrd, f, omega, p, b0, parameters->DN, flag4, &Ldp, &Ldb, &Ld50, &Lbulla50, &Lbulls50, &Ldsph50);
	...
	// double Lb = fmax(Lb0p, Lbd50); // eq (69)
